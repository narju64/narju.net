...
async function addMissingLists() {
  try {
    console.log('🔍 Checking for existing lists...');
    
    // Check if routine list exists
    const routineCheck = await pool.query('SELECT * FROM lists WHERE category = $1', ['routine']);
    if (routineCheck.rows.length === 0) {
      console.log('➕ Adding routine list...');
      const routineQuery = `
      ...
      console.log('ℹ️ Routine list already exists');
    }

    // Check if exercise list exists
    const exerciseCheck = await pool.query('SELECT * FROM lists WHERE category = $1', ['exercise']);
    if (exerciseCheck.rows.length === 0) {
      console.log('➕ Adding exercise list...');
      const exerciseQuery = `
        INSERT INTO lists (name, category, items_json, order_index)
        VALUES ($1, $2, $3, $4)
        RETURNING id;
      ...
      console.log('ℹ️ Exercise list already exists');
    }

    // Check if diet list exists
    const dietCheck = await pool.query('SELECT * FROM lists WHERE category = $1', ['diet']);
    if (dietCheck.rows.length === 0) {
      console.log('➕ Adding diet list...');
      const dietQuery = `
        INSERT INTO lists (name, category, items_json, order_index)
        VALUES ($1, $2, $3, $4)
        RETURNING id;
      ...
      console.log('ℹ️ Diet list already exists');
    }

    console.log('🎉 All missing lists have been added!');
  } catch (error) {
    console.error('❌ Error adding missing lists:', error);
  } finally {
    await pool.end();
  }
}
